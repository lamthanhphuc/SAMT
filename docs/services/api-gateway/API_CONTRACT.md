...
| `PUT /api/identity/profile` | Strip `/api/identity` + Mint internal JWT | `PUT /profile` + `Authorization: Bearer <internal-jwt>` | Protected |
...
**Client Request (Update Profile):**
```http
PUT /api/identity/profile HTTP/1.1
Host: gateway:8080
Authorization: Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6ImlkZW50aXR5LTEiLCJ0eXAiOiJKV1QifQ...
