...
```json
{
  ...
  "path": "/profile",
  "correlationId": "c6f8e0ff-7a7d-4a17-a4f3-f11ef5f2ab4a"
}
```
...
**Business Rules:**
- **BR-PROFILE-GET-01 (MUST):** User identity is derived from the authenticated security context, not from client-supplied user ID
- **BR-PROFILE-GET-02 (MUST):** Endpoint returns the persisted user record from the database
- **BR-PROFILE-GET-03 (MUST):** Role is returned from stored user data and remains read-only
...
```json
{
  ...
  "path": "/profile",
  "correlationId": "9d5ae443-b1f9-4688-af8b-20c6ad3f7fd6"
}
```
...
// 409 - Email already exists
{
  ...
  "status": 409,
  "detail": "Email already registered: updated.student@university.edu"
}
```

**Business Rules:**
- **BR-PROFILE-PUT-01 (MUST):** User identity is derived from the authenticated security context, not from client input
- **BR-PROFILE-PUT-02 (MUST):** Only safe mutable fields are accepted: `email`, `fullName`
- **BR-PROFILE-PUT-03 (MUST NOT):** `id`, `role`, `status`, and audit timestamps are not client-editable
- **BR-PROFILE-PUT-04 (MUST):** Email remains globally unique across users
- **BR-PROFILE-PUT-05 (SHOULD):** Profile updates create an audit entry with old and new values
